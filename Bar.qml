...
Item {
  ...
  function readableOn(surface, preferred) {
    ...
    return contrastRatio(surface, light) >= contrastRatio(surface, dark) ? light : dark
  }
  ...
  function togglePills() {
    ...
    var next = !root.pillsEnabled
    root.shell.mutateShellConfig(function(config) {
      if (!Util.isPlainObject(config.bar)) config.bar = {}
      config.bar.pills = next
    })
  }
  ...
  component ModuleSlot: Item {
    ...
    implicitWidth: hasContent ? (root.vertical ? root.barSize : activeItem.implicitWidth + (pillGap + pillPadding) * 2) : 0
    implicitHeight: hasContent ? (root.vertical ? activeItem.implicitHeight + (pillGap + pillPadding) * 2 : Math.max(root.barSize, activeItem.implicitHeight)) : 0
    width: implicitWidth
    ...
    Rectangle {
      ...
      height: Math.max(0, parent.height - (root.vertical ? slot.pillGap * 2 : Style.space(4)))
      radius: Math.min(width, height) / 2
      color: Color.background
      border.color: Util.alpha(root.barForeground, 0.24)
      border.width: 1
    }
    ...
    Loader {
      ...
      anchors.fill: parent
      anchors.leftMargin: root.vertical ? 0 : slot.pillGap + slot.pillPadding
      anchors.rightMargin: root.vertical ? 0 : slot.pillGap + slot.pillPadding
      anchors.topMargin: root.vertical ? slot.pillGap + slot.pillPadding : 0
      anchors.bottomMargin: root.vertical ? slot.pillGap + slot.pillPadding : 0
